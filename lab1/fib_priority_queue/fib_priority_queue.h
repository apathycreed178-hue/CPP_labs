...
public:
    ...
    void merge(mergeable_priority_queue * to_merge_from) override {
        ...
        do {
            ...
            if (runner->brother == this->_root) {
                runner->brother = fib_to_merge_from->_root;
                runner = fib_to_merge_from->_root;
            }

            if (runner->brother == fib_to_merge_from->_root) {
                runner->brother = this->_root;
                runner = this->_root;
            }

        } while (runner != this->_root);

        _root = (_priorities_comparer(this->_root->priority, fib_to_merge_from->_root->priority) > 0)
            ? this->_root : fib_to_merge_from->_root;
